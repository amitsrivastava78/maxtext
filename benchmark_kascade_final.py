...
def prefill_build_kv_caches(params, input_ids, schedule=None,
    ...
    # JIT the layer function INLINE (not as decorator) so we can del it
    # after the loop to free XLA compilation cache & scratch buffers.
...
def _dense_layer_fn_jit_safe(x, attn_norm_scale, wq, wk, wv, wo,
    ...
    """Layer using chunked attention (no Tokamax). NOT decorated @jax.jit.
...
def main():
    ...
    if args.decode or args.decode_only:
        ...
        if total_decode_gb > 30:
            print(f"\n  ⚠ Decode S={DECODE_SEQ_LEN}: B=1 KV + model + hot bufs = {total_decode_gb:.1f} GB > 30 GB")
        ...
        print(f"   Hot buffer:     [1, {NUM_HEADS}, {sparse_tokens}, {HEAD_DIM}] per REUSE layer (B=1, einsum broadcasts)")
        ...
        for layer_id in tile_indices_map:
            ...
            for qi in range(n_q_tests):
                q_xk = jax.random.normal(
                    ...
                    (1, NUM_HEADS, 1, HEAD_DIM), dtype=COMPUTE_DTYPE)
